(function( $ ) {
	...
	`; id="minggu"
	var weekDaysFromSunday = '<div id="minggu">M</div><div id="senin">S</div><div id="selasa">S</div><div id="rabu">R</div><div id="kamis">K</div><div id="jumat">J</div><div id="sabtu">S</div>';
	var weekDaysFromMonday = '<div>M</div><div>T</div><div>W</div><div>T</div><div>F</div><div>S</div><div>S</div>';
	var shortMonths = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli","Agustus", "September", "Oktober", "November", "Desember"];
    ...
    $.fn.miniEventCalendar = $.fn.MEC = function(options) {
		...
		miniCalendar.on("click touchstart", ".a-date", function(e){
			var day ='';
			var monthYear = $('#monthYear').attr('class');
			var selectedDate = $(this).text();
			var selectedMonth = monthYear.split('-')[0];
			var selectedYear = monthYear.split('-')[1];
			var clickDate = new Date(selectedYear, selectedMonth-1, selectedDate);
			
			switch (clickDate.getDay()) {
			case 0:
				day = "Minggu";
				break;
			case 1:
            ...
            $('.date-header').text( day +', ' + $(this).find('span').text() + ' '+ $('#monthYear').text());
			e.preventDefault(); 
			...
			$.ajax({
				...
				success: function(data){
					$('.list-agenda').remove();
					$('#table-day-agenda').append($(data).find('.list-agenda'));
				},
		...
		function populateCalendar(month, year, onInit) {
			tbody.html("");
			calTitle.removeClass();
			calTitle.addClass(''+parseInt(month+1) + '-' +year);
