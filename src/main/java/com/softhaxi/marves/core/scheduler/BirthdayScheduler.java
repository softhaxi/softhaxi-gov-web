...
public class BirthdayScheduler {
    ...
    public void saveBirthdayNotification() {
        ...
        data.forEach(item -> {
            ...
            Notification notification = new Notification().level("PUBLIC").assignee("ALL").category("BIRTHDAY")
            ...
            if (oneSignalIds != null && !oneSignalIds.isEmpty()) {
                Map<String, Object> body = new HashMap<>(Map.ofEntries(
                        ...
                        entry("contents",
                                Map.of("en", String.format("Sdr/i %s (%s) berulang tahun hari ini", name, division))),
                        entry("data",
                                Map.of("deepLink", notification.getDeepLink(), "view", "detail", "refId",
                                        notification.getId())),
                        entry("include_player_ids", oneSignalIds), entry("small_icon", "ßic_stat_marves"),
